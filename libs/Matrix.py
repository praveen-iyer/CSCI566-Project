...
class VAE_4x(nn.Module):
    ...
    def reparameterize(self, k, l, m, n, a_mu, a_logvar, b_mu, b_logvar):
        a_mu = self.bn(a_mu)
        ...
        std = torch.pow(k * a_var + l * b_var,0.5) #+ m * c_var + n * d_var, 0.5)

        return mu + std

    def forward(self, k, l, m, n, a, b):
        # 32x8x8
        batch,cl,h = a.size()
        a = a.view(batch, -1)
        b = b.view(batch, -1)
        ...
        a_mu, b_mu = mu.chunk(4, dim=0)
        a_logvar, b_logvar = logvar.chunk(4, dim=0)
        # reparameterize
        z_q = self.reparameterize(k, l, m, n, a_mu, a_logvar, b_mu, b_logvar)
        out = self.decode(z_q)
...
class MulLayer_4x(nn.Module):
    ...
    def forward(self, k, l, m, n, cF, sF_1, sF_2, sF_3, sF_4, trans=True):
        ...
        sMean = self.VAE(k, l, m, n, sMean_1, sMean_2)
        sMean = sMean.unsqueeze(3)
        ...
        if(trans):
            ...
            sMatrix = self.snet(k * sF_1 + l*sF_2 )
