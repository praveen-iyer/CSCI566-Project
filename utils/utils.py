...
def generate_out_img_name(config):
    ...
    if 'reconstruct_script' in config:
        ...
        suffix = f'_o_{config["optimizer"]}_i_{config["init_method"]}_h_{str(config["height"])}_m_{config["model"]}_cw_{config["content_weight"]}_s1w_{config["style1_weight"]}_s2w_{config["style2_weight"]}_tv_{config["tv_weight"]}{config["img_format"][1]}'
